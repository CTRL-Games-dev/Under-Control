...
public class InventoryPanel : MonoBehaviour
{
    ...
    public void TryMoveSelectedItem() {
        ...
        if (item.GetComponent<ItemUI>().CurrentInventoryPanel.IsSellerInventory) {
            Player.PlayerController.Coins += (IsItemJustBought ? _selectedInventoryItem.ItemData.Value : _selectedInventoryItem.ItemData.Value / 2) * _selectedInventoryItem.Amount;
        }
    ...
    public void OnItemUIClick(ItemUI itemUI) {
        ...
        if (_inventory.Contains(itemUI.InventoryItem)) {
            if (Player.UICanvas.SelectedItemUI.InventoryItem != null) return; 
            if (Player.PlayerController.Coins - itemUI.InventoryItem.ItemData.Value * itemUI.InventoryItem.Amount < 0 && IsSellerInventory) return;
            Player.UICanvas.SetSelectedItemUI(itemUI);
            if (IsSellerInventory) {
                Player.PlayerController.Coins -= itemUI.InventoryItem.ItemData.Value * itemUI.InventoryItem.Amount;
    ...
    public void OnItemUILeftClick(ItemUI itemUI) {
        ...
        if (_inventory.Contains(itemUI.InventoryItem)) {
            if (Player.UICanvas.SelectedItemUI.InventoryItem == null) {
                ...
                Player.UICanvas.SetSelectedItemUI(itemUI);
                if (IsSellerInventory) {
                    if (Player.PlayerController.Coins - itemUI.InventoryItem.ItemData.Value * itemUI.InventoryItem.Amount < 0) return;
                    Player.PlayerController.Coins -= itemUI.InventoryItem.ItemData.Value * itemUI.InventoryItem.Amount;
                    IsItemJustBought = true;
                }
                destroyAndRemoveItemUI(itemUI.InventoryItem);
                ...
                if (Player.UICanvas.SelectedItemUI.InventoryItem.ItemData == itemUI.InventoryItem.ItemData) {
                    if (itemUI.InventoryItem.ItemData.MaxQuantity - itemUI.InventoryItem.Amount >= Player.UICanvas.SelectedItemUI.InventoryItem.Amount) {
                        ...
                        if (IsSellerInventory) {
                            Player.PlayerController.Coins -= itemUI.InventoryItem.ItemData.Value * Player.UICanvas.SelectedItemUI.InventoryItem.Amount;
                            IsItemJustBought = true;
                        }
                        itemUI.UpdateAmount();
                        Player.UICanvas.SelectedItemUI.InventoryItem = null;
                        ...
                        if (IsSellerInventory) {
                            Player.PlayerController.Coins -= itemUI.InventoryItem.ItemData.Value * (itemUI.InventoryItem.ItemData.MaxQuantity - itemUI.InventoryItem.Amount);
                            IsItemJustBought = true;
                        }
                    }
                }
        ...
        if (_inventory.Contains(itemUI.InventoryItem)) {
            if (Player.UICanvas.SelectedItemUI.InventoryItem == null) {
                if (itemUI.InventoryItem.Amount > 1) {
                    ...
                    if (Player.PlayerController.Coins - itemUI.InventoryItem.ItemData.Value * (half + itemUI.InventoryItem.Amount % 2) < 0 && IsSellerInventory) return;
                    ...
                    itemUI.UpdateAmount();
                    if (IsSellerInventory) {
                        Player.PlayerController.Coins -= itemUI.InventoryItem.ItemData.Value * (half + itemUI.InventoryItem.Amount % 2);
                        IsItemJustBought = true;
                    }
                } else {
                    if (Player.PlayerController.Coins - itemUI.InventoryItem.ItemData.Value * itemUI.InventoryItem.Amount < 0 && IsSellerInventory) return;
                ...
                if (Player.UICanvas.SelectedItemUI.InventoryItem.ItemData == itemUI.InventoryItem.ItemData) {
                    if (itemUI.InventoryItem.Amount + 1 <= itemUI.InventoryItem.ItemData.MaxQuantity && Player.UICanvas.SelectedItemUI.InventoryItem.Amount - 1 >= 0) {
                        ...
                        Player.UICanvas.SelectedItemUI.UpdateAmount();
                        if (IsSellerInventory) {
                            Player.PlayerController.Coins += itemUI.InventoryItem.ItemData.Value;
                            IsItemJustBought = true;
                        }
                    }
                }
